...
/**
 * customer_hazards — einzelne Gefährdung einer Beurteilung.
 *
 * Verweist OPTIONAL per morphMap auf einen Katalog-Eintrag (catalog_type/catalog_id) —
 * die „empfohlene Vorsorge" (z. B. arbmedvv_occasion).
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('customer_hazards', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('team_id')->index();
            $table->foreignId('risk_assessment_id')
                ->constrained('customer_risk_assessments')->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists('customer_hazards');
    }
};
